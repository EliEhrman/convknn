"""
Module descended from dimredvis.py

Attempts to isolate centroids that are homogenous

Finds 27D centroids and then evaluates them based on whether there is a clear majority of one class in them
...
for icent in range(c_num_centroids):
	...
	# Shape=[c_num_centroids]
	t_vote_for_this = tf.equal(v_closest_idxs, icent, name='t_vote_for_this')
	# Count the number of trues in the vote_for this tensor
	# Shape=()
	t_vote_count = tf.reduce_sum(tf.cast(t_vote_for_this, tf.float32), name='t_vote_count')

	l_counts = []
	for iclass in range(num_classes):
		t_this_class = tf.equal(v_db_labels, iclass, name='t_this_class')
		t_both_true = tf.logical_and(t_vote_for_this, t_this_class)
		t_both_count = tf.reduce_sum(tf.cast(t_both_true, tf.float32), name='t_vote_count')
		l_counts.append(tf.cond(t_vote_count > 0.0,
					lambda: tf.divide(t_both_count, t_vote_count),
					lambda: tf.zeros([], dtype=tf.float32),
					name='t_avg'))
	t_counts = tf.stack(l_counts)
	l_percents.append(tf.reduce_max(t_counts))

t_percents = tf.stack(l_percents)



# saver = tf.train.Saver({'dimred':W, 'v_sdb':v_sdb})
...
# embedding.tensor_name = v_sdb.name
...
r_percents = sess.run(t_percents)

meta_labels = sess.run(v_db_labels)
tsv_fh = open(os.path.join(LogDir, tsv_path), 'wb')
tsv_writer = csv.writer(tsv_fh, delimiter='\t')
for idx in meta_labels:
	tsv_writer.writerow(str(idx))
tsv_fh.close()
embedding.metadata_path = os.path.join(LogDir, tsv_path)
